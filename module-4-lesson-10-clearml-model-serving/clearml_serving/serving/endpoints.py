...
def _nested_list_type_convertor(inst):  # noqa
    if inst is None:
        return None
    if isinstance(inst, (tuple, list)) and all(
        not isinstance(i, (tuple, list)) for i in inst
    ):
        return [inst]
...
class ModelMonitoring(BaseStruct):
    base_serving_url = attrib(
        type=str
    )  # serving point url prefix (example: "detect_cat")
    engine_type = attrib(type=str, validator=_engine_validator)  # engine type
    monitor_project = attrib(
        type=str, default=None
    )  # monitor model project (for model auto update)
    monitor_name = attrib(
        type=str, default=None
    )  # monitor model name (for model auto update, regexp selection)
    monitor_tags = attrib(
        type=list, default=[]
    )  # monitor model tag (for model auto update)
    only_published = attrib(type=bool, default=False)  # only select published models
    max_versions = attrib(
        type=int, default=None
    )  # Maximum number of models to keep serving (latest X models)
    input_size = attrib(
        type=list, default=None, converter=_nested_list_type_convertor
    )  # optional,  model matrix size
    input_type = attrib(
        type=list,
        default=None,
        validator=_matrix_type_validator,
        converter=_list_type_convertor,
    )
    input_name = attrib(
        type=list, default=None, converter=_list_type_convertor
    )  # optional, input layer names
    output_size = attrib(
        type=list, default=None, converter=_nested_list_type_convertor
    )  # optional, model matrix size
    output_type = attrib(
    ...
    output_name = attrib(
        type=list, default=None, converter=_list_type_convertor
    )  # optional, output layer names
    preprocess_artifact = attrib(
        type=str, default=None
    )  # optional artifact name storing the model preprocessing code
    auxiliary_cfg = attrib(
        type=dict, default=None
    )  # Auxiliary configuration (e.g. triton conf), Union[str, dict]
...
class ModelEndpoint(BaseStruct):
    ...
    serving_url = attrib(
        type=str
    )  # full serving point url (including version) example: "detect_cat/v1"
    ...
    auxiliary_cfg = attrib(
        type=dict, default=None
    )  # Optional: Auxiliary configuration (e.g. triton conf), [str, dict]
...
class CanaryEP(BaseStruct):
    ...
    weights = attrib(
        type=list
    )  # list of weights (order should be matching fixed_endpoints or prefix)
    load_endpoints = attrib(
        type=list, default=[]
    )  # list of endpoints to balance and route
    load_endpoint_prefix = attrib(type=str, default=None)  # endpoint prefix to list
...
class EndpointMetricLogging(BaseStruct):
    ...
    class MetricType(BaseStruct):
        type = attrib(
            type=str, validator=validators.in_(("scalar", "enum", "value", "counter"))
        )
        buckets = attrib(type=list, default=None)

    endpoint = attrib(
        type=str
    )  # Specific endpoint to log metrics w/ version (example: "model/1")
    # If endpoint name ends with a "*" any endpoint with a matching prefix will be selected

    log_frequency = attrib(
        type=float, default=None
    )  # Specific endpoint to log frequency
    ...
    metrics = attrib(
        ...
        converter=lambda x: {
            k: v
            if isinstance(v, EndpointMetricLogging.MetricType)
            else EndpointMetricLogging.MetricType(**v)
            for k, v in x.items()
        },
    ...
    def as_dict(self, remove_null_entries=False):
        if not remove_null_entries:
            return {
                k: v.as_dict(remove_null_entries) if isinstance(v, BaseStruct) else v
