...
from clearml_serving.serving.endpoints import (
    ModelMonitoring,
    ModelEndpoint,
    ...
    if stored_v != current_v:
        ...
        if answer_yes:
            ...
            if should_continue not in ("y", "yes"):
                print(
                    "INFO: If you wish to downgrade your clearml-serving CLI use "
                    "`pip3 install clearml-serving=={}`".format(
                        request_processor.get_version()
                    )
                )
                sys.exit(0)
...
def func_metric_ls(args):
    ...
    print(
        "List endpoint metrics, control task id={}".format(request_processor.get_id())
    )
    ...
    print(
        "Logged Metrics:\n{}".format(
            json.dumps(
                {
                    k: v.as_dict()
                    for k, v in request_processor.list_metric_logging().items()
                },
                indent=2,
            )
        )
...
def func_metric_rm(args):
    ...
    print(
        "Serving service Task {}, Removing metrics from endpoint={}".format(
    ...
    if not args.variable:
        if request_processor.remove_metric_logging(endpoint=args.endpoint):
            ...
            raise ValueError(
                "Could not remove metric endpoint {}".format(args.endpoint)
            )
    else:
        for v in args.variable:
            if request_processor.remove_metric_logging(
                endpoint=args.endpoint, variable_name=v
            ):
                print("Removing metric endpoint: {} / {}".format(args.endpoint, v))
            else:
                raise ValueError(
                    "Could not remove metric {} from endpoint {}".format(
                        v, args.endpoint
                    )
                )
...
def func_metric_add(args):
    request_processor = safe_ModelRequestProcessor(task_id=args.id)
    print(
        "Serving service Task {}, Adding metric logging endpoint '/{}/'".format(
            request_processor.get_id(), args.endpoint
        )
    )
    request_processor.deserialize(skip_sync=True)
    ...
    for v in args.variable_scalar or []:
        if "=" not in v:
            raise ValueError(
                "Variable '{}' should be in the form of <name>=<buckets> "
    ...
    if not request_processor.add_metric_logging(metric=metric, update=True):
        raise ValueError(
            "Could not add metric logging endpoint {}".format(args.endpoint)
        )
...
def func_model_upload(args):
    ...
    print(
        "Creating new Model name='{}' project='{}' tags={}".format(
            args.name, args.project, args.tags or ""
        )
    )
    model = OutputModel(
        task=t, name=args.name, tags=args.tags or None, framework=args.framework
    )
    destination = (
        args.destination
        or t.get_output_destination()
        or t.get_logger().get_default_upload_destination()
    )
    ...
    model.update_weights(
        weights_filename=args.path, register_uri=args.url, auto_delete_file=False
    )
    ...
    if args.project:
        ...
        model._base_model.update(
            project_id=get_or_create_project(
                session=t.session, project_name=args.project
            )
        )
...
def func_model_ls(args):
    request_processor = ModelRequestProcessor(task_id=args.id)
    print(
        "List model serving and endpoints, control task id={}".format(
            request_processor.get_id()
        )
    )
    request_processor.deserialize(skip_sync=True)
    print(
        "Endpoints:\n{}".format(
            json.dumps(
                {k: v.as_dict() for k, v in request_processor.get_endpoints().items()},
                indent=2,
            )
        )
    )
    print(
        "Model Monitoring:\n{}".format(
            json.dumps(
                {
                    k: v.as_dict()
                    for k, v in request_processor.get_model_monitoring().items()
                },
                indent=2,
            )
        )
    ...
    print(
        "Canary:\n{}".format(
            json.dumps(
                {
                    k: v.as_dict()
                    for k, v in request_processor.get_canary_endpoints().items()
                },
                indent=2,
            )
        )
...
def func_config_service(args):
    ...
    if args.base_serving_url:
        print(
            "Configuring serving service [id={}] base_serving_url={}".format(
    ...
    if args.triton_grpc_server:
        print(
            "Configuring serving service [id={}] triton_grpc_server={}".format(
    ...
    if args.kafka_metric_server:
        request_processor.configure(
            external_kafka_service_server=args.kafka_metric_server
        )
...
def func_model_remove(args):
    request_processor = safe_ModelRequestProcessor(task_id=args.id)
    print(
        "Serving service Task {}, Removing Model endpoint={}".format(
            request_processor.get_id(), args.endpoint
        )
    )
    request_processor.deserialize(skip_sync=True)
...
def func_canary_add(args):
    request_processor = safe_ModelRequestProcessor(task_id=args.id)
    print(
        "Serving service Task {}, Adding canary endpoint '/{}/'".format(
            request_processor.get_id(), args.endpoint
        )
    )
    request_processor.deserialize(skip_sync=True)
...
def func_model_auto_update_add(args):
    request_processor = safe_ModelRequestProcessor(task_id=args.id)
    print(
        "Serving service Task {}, Adding Model monitoring endpoint: '/{}/'".format(
            request_processor.get_id(), args.endpoint
        )
    )
...
def func_model_endpoint_add(args):
    request_processor = safe_ModelRequestProcessor(task_id=args.id)
    print(
        "Serving service Task {}, Adding Model endpoint '/{}/'".format(
            request_processor.get_id(), args.endpoint
        )
    )
    request_processor.deserialize(skip_sync=True)
...
def cli():
    ...
    parser.add_argument(
        "--yes", action="store_true", help="Always answer YES on interactive inputs"
    )
    parser.add_argument(
        "--id",
        type=str,
    ...
    parser_metrics = subparsers.add_parser(
        "metrics", help="Configure inference metrics Service"
    )
    ...
    parser_metrics_add = metric_cmd.add_parser(
        "add", help="Add/modify metric for a specific endpoint"
    )
    ...
    parser_metrics_rm = metric_cmd.add_parser(
        "remove", help="Remove metric from a specific endpoint"
    )
    ...
    parser_metrics_ls = metric_cmd.add_parser(
        "list", help="list metrics logged on all endpoints"
    )
    parser_metrics_ls.set_defaults(func=func_metric_ls)

    parser_config = subparsers.add_parser(
        "config", help="Configure a new Serving Service"
    )
    ...
    parser_model = subparsers.add_parser(
        "model", help="Configure Model endpoints for an already running Service"
    )
    ...
    parser_model_rm = model_cmd.add_parser(
        "remove", help="Remove model by it`s endpoint name"
    )
    parser_model_rm.add_argument("--endpoint", type=str, help="model endpoint name")
    parser_model_rm.set_defaults(func=func_model_remove)

    parser_model_upload = model_cmd.add_parser(
        "upload", help="Upload and register model files/folder"
    )
    ...
    parser_model_upload.add_argument(
        ...
        choices=[
            p
            for p in Framework.__dict__.keys()
            if not p.startswith("_") and not callable(getattr(Framework, p))
        ]
        + ["custom"],
    ...
    parser_model_lb = model_cmd.add_parser(
        "canary", help="Add model Canary/A/B endpoint"
    )
    parser_model_lb.add_argument(
        "--endpoint",
        type=str,
        help="model canary serving endpoint name (e.g. my_model/latest)",
    )
    parser_model_lb.add_argument(
        "--weights",
        type=float,
        nargs="+",
        help="model canary weights (order matching model ep), (e.g. 0.2 0.8)",
    )
    parser_model_lb.add_argument(
        "--input-endpoints",
        type=str,
    ...
    parser_model_monitor = model_cmd.add_parser(
        "auto-update", help="Add/Modify model auto update service"
    )
    parser_model_monitor.add_argument(
        "--endpoint", type=str, help="Base Model endpoint (must be unique)"
    )
    parser_model_monitor.add_argument(
        "--engine",
        type=str,
    ...
    parser_model_add.add_argument(
        "--endpoint", type=str, required=True, help="Model endpoint (must be unique)"
    )
    parser_model_add.add_argument(
        "--version",
        type=str,
    ...
    parser_model_add.add_argument(
        "--model-id", type=str, help="Specify a Model ID to be served"
    )
    parser_model_add.add_argument(
        "--preprocess",
        type=str,
